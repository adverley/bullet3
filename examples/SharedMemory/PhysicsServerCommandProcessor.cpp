...
bool PhysicsServerCommandProcessor::processLoadSoftWireCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes)
{
	const LoadSoftWireArgs& loadSoftBodyArgs = clientCmd.m_loadSoftWireArguments;
	bool hasStatus = true;
	...
	if (clientCmd.m_updateFlags & LOAD_SOFT_WIRE_BOXES)
	{
		total_boxes = clientCmd.m_loadSoftWireArguments.m_num_boxes;
	}
	if (clientCmd.m_updateFlags & LOAD_SOFT_WIRE_SCALE)
	{
		scale = clientCmd.m_loadSoftWireArguments.m_scale;
	}
	if (clientCmd.m_updateFlags & LOAD_SOFT_WIRE_INITIAL_POSITION)
	{
		initialPos[0] = clientCmd.m_loadSoftWireArguments.m_initialPosition[0];
		initialPos[1] = clientCmd.m_loadSoftWireArguments.m_initialPosition[1];
		initialPos[2] = clientCmd.m_loadSoftWireArguments.m_initialPosition[2];
	}
	if (clientCmd.m_updateFlags & LOAD_SOFT_WIRE_INITIAL_ORIENTATION)
	{
		initialOrn[0] = clientCmd.m_loadSoftWireArguments.m_initialOrientation[0];
		initialOrn[1] = clientCmd.m_loadSoftWireArguments.m_initialOrientation[1];
		initialOrn[2] = clientCmd.m_loadSoftWireArguments.m_initialOrientation[2];
		initialOrn[3] = clientCmd.m_loadSoftWireArguments.m_initialOrientation[3];
	}

	double size = 10.0 / total_boxes;
	btCollisionShape* colShape = worldImporter->createBoxShape(btVector3(0.025, 0.1 * size, 0.025) * btScalar(scale));
	...
	double phi = initialOrn.getAngle();
	double theta = initialOrn.getAngleShortestPath();
	for (int i = 0; i<total_boxes; ++i) {
		startTransform.setOrigin(btVector3(
			btCos(btScalar(phi)) * btSin(btScalar(theta)),
			btSin(btScalar(phi)) * btSin(btScalar(theta)),
			btCos(btScalar(theta))
		) * btScalar(0.1 * i * 2 * size * scale) + initialPos
		);
		startTransform.setRotation(initialOrn);
		boxes.push_back(worldImporter->createRigidBody((i == lastBoxIndex) ? 1 : 1, (i == lastBoxIndex) ? mass : mass, startTransform, colShape, 0));
	}
	// If you want to spawn middle: two for loops for half amount of blocks, start one that goes in the opposite direction of the other
	...
	for (int i = 0; i<total_boxes - 1; ++i) {
		...
		btPoint2PointConstraint* leftSpring = new btPoint2PointConstraint(*b1, *b2, btVector3(-0.05, 0.1 * size, 0) * btScalar(scale), btVector3(-0.05, -0.1 * size, 0) * btScalar(scale));
		//leftSpring->m_setting.m_tau = 1.;
		leftSpring->m_setting.m_damping = 0.97;
		m_data->m_dynamicsWorld->addConstraint(leftSpring);

		btPoint2PointConstraint* rightSpring = new btPoint2PointConstraint(*b1, *b2, btVector3(0.05, 0.1 * size, 0) * btScalar(scale), btVector3(0.05, -0.1 * size, 0) * btScalar(scale));
		//rightSpring->m_setting.m_tau = 1.;
		rightSpring->m_setting.m_impulseClamp = 0.97;
		m_data->m_dynamicsWorld->addConstraint(rightSpring);
	}
...
bool PhysicsServerCommandProcessor::processCommand(const struct SharedMemoryCommand& clientCmd, struct SharedMemoryStatus& serverStatusOut, char* bufferServerToClient, int bufferSizeInBytes )
{
	...
	case CMD_LOAD_SOFT_WIRE:
		{
			hasStatus = processLoadSoftWireCommand(clientCmd, serverStatusOut, bufferServerToClient, bufferSizeInBytes);
			break;
		}
