...
class RewardZoo():
    ...
    def create_function(cls, REWARD_TYPE):
        ...
        elif(REWARD_TYPE == 'advanced_sparse_reward'):
            return cls.advanced_sparse_reward
        else:
    ...
    def advanced_sparse_reward(env):
        """Calculates the reward for the episode.
        """
        torus_pos = np.array(
            p.getBasePositionAndOrientation(env._baxter.torusUid)[0])
        ...
        y_bool = (torus_pos[1] - env._baxter.torusRad) < block_pos[1] and (torus_pos[1] + env._baxter.torusRad) > \
                 block_pos[1]
        z_bool = (torus_pos[2] - env._baxter.torusRad) < block_pos[2] and (torus_pos[2] + env._baxter.torusRad) > \
                 block_pos[2]
