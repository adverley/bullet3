...
class KukaGymEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if (self._isDiscrete):
            ...
            self.action_space = spaces.Discrete(len(self._kuka.motorIndices))
        else:
            action_dim = 3
    ...
    def _reset(self):
        ...
        self.blockUid = p.loadURDF(os.path.join(
            self._urdfRoot, "block.urdf"), xpos, ypos, -0.15, orn[0], orn[1], orn[2], orn[3])
    ...
    def _step(self, action):
        ...
        realAction = []
        dv = 0.005
        for i in range(len(self._kuka.motorIndices)):
            realAction.append(action[i] * dv)
    ...
    def _termination(self):
        ...
        if (len(closestPoints)):  # (actualEndEffectorPos[2] <= -0.43):
            ...
            fingerAngle = 0.3
            # Reset kuka to use inverse kinematics for the next part
            self._kuka.useInverseKinematics = 1
            for i in range(100):
