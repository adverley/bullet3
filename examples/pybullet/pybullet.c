...
static PyObject* pybullet_loadSoftWire(PyObject* self, PyObject* args, PyObject* keywds)
{
	b3PhysicsClientHandle sm = 0;
	int physicsClientId = 0;
	int flags = 0;
	...
	static char* kwlist[] = { "num_boxes", "scale", "basePosition", "baseOrientation", "physicsClientId", NULL };
	...
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "idOO|i", kwlist, &num_boxes, &scale, &basePosObj, &baseOrnObj, &physicsClientId))
	{
		return NULL;
	}
	else
	...
	b3SharedMemoryStatusHandle statusHandle;
	int statusType;
	b3SharedMemoryCommandHandle command = b3CreateSoftWireCommandInit(sm);

	if (num_boxes>0)
	{
		b3CreateSoftWireCommandSetBoxes(command, num_boxes);
	}
	if (scale>0.0)
	{
		b3CreateSoftWireCommandSetScale(command, scale);
	}
	/*
	if (collisionMargin>0)
	{
	...
	*/
	...
	// setting the initial position, orientation and other arguments are
	b3CreateSoftWireCommandSetStartPosition(command, startPosX, startPosY, startPosZ);
	b3CreateSoftWireCommandSetStartOrientation(command, startOrnX, startOrnY, startOrnZ, startOrnW);

	statusHandle = b3SubmitClientCommandAndWaitStatus(sm, command);
	statusType = b3GetStatusType(statusHandle);
	if (statusType != CMD_LOAD_SOFT_WIRE_COMPLETED)
	{
		PyErr_SetString(SpamError, "Cannot load soft wire.");
		return NULL;
	}
	bodyUniqueId = b3GetStatusBodyIndex(statusHandle);
...
static PyMethodDef SpamMethods[] = {
	...
	{"loadSoftBody", (PyCFunction)pybullet_loadSoftBody, METH_VARARGS | METH_KEYWORDS,
	 "Load a softbody from an obj file."},
#endif
	{"loadSoftWire", (PyCFunction)pybullet_loadSoftWire, METH_VARARGS | METH_KEYWORDS,
	 "Load an emulation of a soft body wire."},
